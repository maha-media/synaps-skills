# validation — `skill validate` and `skill lint`
...
```bash
skill validate                          # plugin (or all under) cwd
skill validate ./web-tools-plugin       # one plugin
skill validate ~/Projects/synaps-skills # whole monorepo
```
...
```bash
skill lint                              # warnings only, exit 0
skill lint --strict                     # warnings → errors, exit 1 if any
skill lint ./web-tools-plugin           # one plugin
```
...
```bash
skill validate .                        # MUST pass
skill lint --strict .                   # SHOULD pass
```
