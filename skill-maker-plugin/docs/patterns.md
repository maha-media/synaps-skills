...
**Scaffold:**
```bash
skill new plugin my-tool --umbrella --desc "Use for …"
```
...
independently. The agent invokes `skill-a` or `skill-c` directly based
...
```bash
skill new plugin discipline --desc "Use when …"
cd discipline-plugin
skill new skill code-review --desc "Use when reviewing code."
skill new skill tdd          --desc "Use when implementing logic."
```
...
skill-maker plugin uses `bin/skill` + `lib/*.sh` since there's only one
command.

## Adding self-healing memory

Pass `--memory` to `skill new plugin`. You get:
