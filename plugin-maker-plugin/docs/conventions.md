...
`plugin-maker` validate/lint rule IDs are namespaced by the manifest section
they cover. Full table in [`specs/plugin-maker.md`](specs/plugin-maker.md).

| Prefix | Section | Examples |
|---|---|---|
| `P###` | top-level `plugin.json` | `P001` name kebab, `P004` desc length |
| `F###` | folder layout / files   | `F001` SKILL.md present, `F002` plugin dir suffix |
| `B###` | author / repo / license | `B001` author.name set, `B003` license is SPDX |
| `X###` | `extension` block       | `X005` hook→permission match |
| `S###` | `provides.sidecar`      | `S002` valid lifecycle |
| `T###` | `settings.categories`   | `T003` valid field type |
| `K###` | `keybinds[]`            | `K003` reserved key |
| `C###` | `commands[]`            | `C002` mixed shape discriminators |

Lint-severity rules use the same namespace shifted by 100 (e.g. `X101`,
`P101`).
